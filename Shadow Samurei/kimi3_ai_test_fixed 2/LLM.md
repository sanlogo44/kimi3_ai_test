...
| `kern_modul` | `from kern_modul import kern` | Zugriff auf das Rust-Modul `kimi3_kern` |
...
`rust/web` ist die Weboberfläche (axum, Programm `kimi3-web`):
...
- `sitzung.rs` – Sitzung als signiertes Cookie `kimi3_sitzung` (`SECRET_KEY`)
...
`python main.py --modus web` sucht das Programm über `KIMI3_WEB_BINAER`,
