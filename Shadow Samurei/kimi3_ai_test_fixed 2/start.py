...
def _hilfe() -> None:
    ...
    print()
    print("Optionen:")
    print("    --modus <gui|web|cli|ziel>  Startmodus (Standard: gui)")
...
def parse_argumente() -> dict:
    ...
    werte = {"modus": "gui", "port": 5000, "host": "0.0.0.0",
             "kein_venv": False, "kein_rust": False, "hilfe": False}
    ...
    if werte["modus"] not in ("gui", "web", "cli", "ziel"):
        _fehler(
            f"Unbekannter Modus: {werte['modus']} "
            f"(erlaubt: gui, web, cli, ziel)"
        )
        sys.exit(1)
