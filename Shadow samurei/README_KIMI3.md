# Kimi-K3 Integration – homogenes Programm

`kimi3_ai_test` ist jetzt ein einziges homogenes Programm: Das Kimi-K3-Setup-Tooling ist nicht mehr ein separater Ordner, sondern als Python-Subpaket `kimi_k3/` direkt in die App integriert. Ein einziger Aufruf richtet alles ein und startet die Anwendung.
...
python start.py --modus kimi3   # lokale Kimi-K3-Inferenz (nur mit --mit-torch)
...
Bash-Nutzer können alternativ `./run_inference.sh` für die Inferenz aufrufen (dünnen Wrapper um `python -m kimi_k3.inference`).
...
## Das `kimi_k3/`-Subpaket

| Datei | Zweck |
|-------|-------|
| `backend.py` | Hardware-/Treibererkennung + PyTorch-Installation (reines Python, plattformübergreifend) |
| `diagnose.py` | GPU-Verfügbarkeitsdiagnose + Matmul-Smoketest (`python -m kimi_k3.diagnose`) |
| `inference.py` | Lokale Inferenz (transformers) + OpenAI-kompatibler API-Client (`python -m kimi_k3.inference`) |

## Treiberkompatibilität

Vor jeder PyTorch-Installation prüft `kimi_k3.backend` den GPU-Treiber:
...
| `KIMI3_START_KEIN_RUST` | – | `1` = Rust-Bau überspringen |
| `MODEL_ID` | `moonshotai/Kimi-K3` | Modell für den `kimi3`-Modus |
