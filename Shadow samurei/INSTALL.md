...
Weboberfläche. Python greift über das gebaute Modul `shadow_kern` darauf zu,
...
Hinweis: `source venv/bin/activate` funktioniert auf **macOS und Linux**
gleich. Auf macOS kann stattdessen auch `source ./venv/bin/activate`
verwendet werden. Windows nutzt `venv\Scripts\activate` (cmd) bzw.
...
legt das Python-Modul als `shadow_kern.so` in den Projektordner und erzeugt das
Programm der Weboberfläche unter `rust/target/release/shadow-web`. Ohne diesen
Schritt melden die Python-Module beim Start: „Der Rust-Kern „shadow_kern“ wurde
...
hardware:
  ...
  use_4bit: true      # bei wenig Grafikspeicher sinnvoll (nur CUDA)
  weights_dtype: "fp32"
  npu:
    enabled: true     # Intel NPU (OpenVINO) / Huawei Ascend nutzen
  tpu:
    enabled: true     # Google Cloud TPU nutzen
  parallel:
    enabled: true     # mehrere Aufgaben gleichzeitig trainieren
    max_workers: 4
```

Geräteübersicht: `cuda` (NVIDIA), `mps` (Apple Silicon), `npu` (Intel/Ascend),
`tpu` (Google Cloud), `xpu` (Intel/AMD-Grafik), `cpu` (immer verfügbar).
Mit `device: "auto"` wählt Shadow automatisch das erste verfügbare Gerät;
NPU, TPU, CUDA-Karte und MPS können im selben Lauf gemeinsam genutzt werden.
...
**`Der Rust-Kern „shadow_kern“ wurde nicht gefunden`**
...
**`Das Programm der Weboberfläche „shadow-web“ wurde nicht gefunden`**

`python main.py --modus web` startet `rust/target/release/shadow-web`. Nach
`bash rust/bauen.sh` ist es vorhanden; ein anderer Ort lässt sich über die
Umgebungsvariable `SHADOW_WEB_BINAER` angeben.
...
**NPU wird nicht erkannt (Intel)**

OpenVINO-Runtime installieren:

```bash
pip install openvino
```

Danach `device: "npu"` in `config.yaml` setzen (oder `"auto"`). Fehlt der
Treiber, fällt das Training auf das nächste verfügbare Gerät zurück.

**TPU wird nicht erkannt (Google Cloud)**

PJRT-Runtime wird benötigt. Auf Cloud-TPU-VMs ist sie vorhanden; lokal fehlt sie
meist. Ohne PJRT bleibt die TPU deaktiviert, der Rest funktioniert.

**Paralleles Training startet nicht**

`hardware.parallel.enabled: true` in `config.yaml` prüfen. Pro Gerät läuft nur
ein Modell-Schritt gleichzeitig; für echte Parallelität mehrere Geräte oder
`max_workers > 1` einstellen.
