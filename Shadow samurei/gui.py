...
class ChatFenster:
    ...
    def __init__(self) -> None:
        ...
        self.ziel_aktiv = False

        # Warteschlange: LLM-/Tool-Aufträge nacheinander verarbeiten.
        self.warteschlange = AuftragsWarteschlange()
        self.warteschlange.starten()
    ...
    def _beantworte(
        ...
        if self.llm is None:
            ...
            raise RuntimeError(
                ...
                + "\nAktiviere den Chat mit: python start.py --mit-torch"
            )

        if self.ziel_aktiv and self.ziel_modus is not None:
            # Ziel-Modus hat Vorrang: autonomes Arbeiten bis zum Ziel.
            # Der gesamte Ziel-Durchlauf wird als ein Auftrag in die
            # Warteschlange eingereiht, damit aufeinanderfolgende Anfragen
            # geordnet verarbeitet werden.
            ergebnis = self.warteschlange.einreihen(
                "ziel",
                ausfuehren=lambda: self.ziel_modus.arbeite_bis_ziel(
                    frage,
                    verlauf=verlauf,
        ...
        if self.schwarm_aktiv and self.schwarm is not None:
            ...
            # Der gesamte Schwarm-Durchlauf wird als ein Auftrag in die
            # Warteschlange eingereiht.
            ergebnis = self.warteschlange.einreihen(
                "schwarm",
                ausfuehren=lambda: self.schwarm.beantworte(
                    frage,
                    verlauf=verlauf,
                    teilstueck_rueckmeldung=melde_teilstueck,
                    abbruch=abbruch,
        ...
        ergebnis = self.warteschlange.einreihen(
            "chat",
            ausfuehren=lambda: self.llm.chat_with_tools(
                frage,
                self.client,
