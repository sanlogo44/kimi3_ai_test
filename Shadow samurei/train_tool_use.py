...
def train(
    ...
    except (OSError, ValueError, RuntimeError, ConnectionError) as fehler:
        # Modell-Download, gated-Repo (401) oder Netzwerkfehler sauber
        # abfangen – analog zum fehlenden ImportError-Pfad oben.
        protokoll.error(f"Training fehlgeschlagen: {fehler}")
        return {
            "trainiert": False,
            "fehler": str(fehler),
            "beispiele": len(beispiele),
