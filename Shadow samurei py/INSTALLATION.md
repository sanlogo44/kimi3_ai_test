...
Abhängigkeiten installieren und die Desktop-Oberfläche starten.
...
| `--mit-torch` | AI-Abhängigkeiten installieren (Chat aktivieren) |
...
Ohne PyTorch startet die Oberfläche trotzdem; Chat und Training bleiben dann
...
Ziel-Ablauf mit einem Fake-LLM (ohne echtes Modell). `test_ziel_modus_echt.py`
fuehrt zusaetzlich **echte** Pruefungen aus: reale Python-Dateien werden mit
`py_compile` kompiliert, und erzeugter Code wird in einem Unterprozess wirklich
ausgefuehrt und die Ausgabe mit dem erwartetten Wert verglichen.
...
venv\Scripts\activate            # Windows (cmd) / venv\Scripts\Activate.ps1 (PowerShell)
