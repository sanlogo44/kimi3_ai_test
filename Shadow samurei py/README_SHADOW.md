...
Die Anwendung besteht komplett aus Python: der gemeinsame Kern (Ordner `kern/`)
für Konfiguration, Protokoll, Einstellungen, Schalter, Metriken, Bewertungen,
Konten, Checkpoints und Rechner, die Weboberfläche (Ordner `web/`) sowie die
CustomTkinter-Oberflächen und der PyTorch-Modellkern.

## Ordner

```
Shadow/
├── kern/            Python-Kern: Konfiguration, Protokoll, Einstellungen, Schalter,
│                    Metriken, Bewertungen, Konten, Checkpoints, Rechner
├── web/             Weboberfläche (Flask): Routen, Sitzung, Vorlagen, Brücke
...
| `kern_modul` | `from kern_modul import kern` | Zugriff auf das Python-Kern-Paket `kern` |
...
Aliasnamen und Meldungstexte; es gibt keine zweite Fassung derselben Logik.
Neue Regeln oder Formate gehören daher in das Paket `kern/`, nicht in die
Hülle.

## Weboberfläche in Python

`web/` ist die Weboberfläche (Flask):
...
- `bruecke.py` – ruft für PyTorch-Aufgaben `kern_bruecke.py` als kurzen
  Python-Vorgang auf (Zeilenprotokoll aus JSON)

`python main.py --modus web` startet den Flask-Webserver direkt.
...
## Geräte und paralleles Training

`model_manager` wählt das Trainingsgerät über `config.yaml → hardware.device`
(`auto`, `cuda`, `mps`, `npu`, `tpu`, `xpu`, `cpu`). Bei `auto` werden alle
verfügbaren Geräte erkannt; NPU, TPU, CUDA-Karte und MPS können im selben Lauf
gemeinsam genutzt werden. Ist `hardware.parallel.enabled: true`, laufen bis zu
`max_workers` Trainingsaufträge gleichzeitig – verteilt auf mehrere Geräte oder
in getrennten Streams auf einem Gerät. Jeder Auftrag geht durch die
`AuftragsWarteschlange` (siehe [README_WARTESCHLANGE.md](README_WARTESCHLANGE.md));
innerhalb eines einzelnen Modellschritts bleibt die Ausführung nacheinander,
damit sich Streams nicht überlappen.
